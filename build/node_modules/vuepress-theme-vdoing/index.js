  ...
  return {
    ...
    plugins: [
      ...
      ['container', {
        ...
        before: info => {
          let open = "";
          if(info&&info.endsWith("open")){
            info = info.replace("open","");
            open = "open";
          }
          return  `<details class="custom-block details "  `+open+`> ${info ? `<summary>${info}</summary>` : ''}\n`
        },
        after: () => '</details>\n',
        defaultTitle: {
          '/': '点击查看',
...
function getTarget(link,target){
    target = target||''
       let t = '';
    if(target!==''){
        t = 'target="'+ target +'"';
...
function getCardListDOM(dataList, row, config) {
  ...
  dataList.forEach(item => {
    listDOM += `
      <${item.link ? 'a href="' + item.link + '" ' +   getTarget(item.link,item.target) + '"' : 'span'} class="card-item ${row ? 'row-' + row : ''}"
...
function getCardImgListDOM(dataList, row, config) {
  const { imgHeight = 'auto', objectFit = 'cover', lineClamp = 1 } = config

  let listDOM = ''
  dataList.forEach(item => {
    listDOM += `
      <div class="card-item ${row ? 'row-' + row : ''}" >
        <a href="${item.link}" ${getTarget(item.link,item.target)}>
